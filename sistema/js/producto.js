  ...
  $.ajax({
    ...
    //  $('.nameProducto').html(info.descripcion);
      ...
      $('.bodyModal').html('<form action="" method="post" name="form_del_product" id="form_del_product" onsubmit="event.preventDefault(); delProduct();">'+
        '<h2 style="color: red; font-size: 18px;">Are you sure to delete the Product</h2>'+
        '<h2 class="nameProducto">'+info.descripcion+'</h2>'+
        '<hr>'+
        '<input type="hidden" name="producto_id" id="producto_id" value="'+info.codproducto+'" required><br>'+
...
$(".confirmar").submit(function(e) {
  e.preventDefault();
  Swal.fire({
    title: 'You are about to delete an action, are you sure?',
    icon: 'warning',
...
function validaPass() {
  ...
  if (passNuevo != confirmarPass) {
    $('.alertChangePass').html('<p style="color:red;">Passwords do not match</p>');
    $('.alertChangePass').slideDown();
    return false;
  }
if (passNuevo.length < 5) {
  $('.alertChangePass').html('<p style="color:orangered;">Passwords must contain at least 5 characters');
  $('.alertChangePass').slideDown();
  return false;
}

$('.alertChangePass').html('<p style="color:blue;">Passwords match.</p>');
$('.alertChangePass').slideDown();
}
...
function sendDataProduct() {
  ...
  $.ajax({
    ...
    success: function(response) {
      if (producto == 'error') {
        $('.alertAddProduct').html('<p style="color : red;">Error adding product.</p>');
        ...
        $('#txtCantidad').val('');
        $('#txtPrecio').val('');
        $('.alertAddProduct').html('<p>Product Added Successfully.</p>');
...
function delProduct() {
  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if (response == 'error') {
        $('.alertAddProduct').html('<p style="color : red;">Product deletion error.</p>');

      }else {

        $('.row'+pr).remove();
        $('#form_del_product .ok').remove();
        $('.alertAddProduct').html('<p>Properly Disposed Product.</p>');
